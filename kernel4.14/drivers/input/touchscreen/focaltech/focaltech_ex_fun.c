...
int fts_6x36u_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf, u32 dw_lenth);
...
#ifdef S5001_D_FIRMWARE_UPGRADE
	#include "LHJ2024_FT6336U_Ver0x03_20170401_app.i"
#elif defined(D66_A_FIRMWARE_UPGRADE)
	#include "D66_FW_FT6336U_Ver0x03_20170114_app.i"
#elif defined(L606_A_FIRMWARE_UPGRADE)
	#include "L606A(HD)_FT6436_4Columns_Ver0x05_20170519_app.i"
#elif defined(V33K_FIRMWARE_UPGRADE)
	#include "HC055F312_duoweijike_640X1280_FT6336U_Ver0x02_20171024_app.i"
#endif
};
#else
static unsigned char CTPM_FW[] = {
...
#include "Y138066A1_Ver0x01_20171018_app.i"
};
#endif
...
#ifdef V33K_FIRMWARE_UPGRADE_2
 	else if ((fts_updateinfo_curr.CHIP_ID==0x64)&&(fts_updateinfo_curr.VENDOR_ID==0x11))
	{
		printk("%s : CHIP_ID == 0x64! VENDOR_ID==0x11\n",__func__);
		...
		pbt_buf = CTPM_FW;
		i_ret = fts_6x36u_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
	}
		else if ((fts_updateinfo_curr.CHIP_ID==0x64)&&(fts_updateinfo_curr.VENDOR_ID==0x7a))
	{
		printk("%s : CHIP_ID == 0x64!  VENDOR_ID==0x7a \n",__func__);
		...
		pbt_buf = CTPM_FW2;
		i_ret = fts_6x36u_ctpm_fw_upgrade(client, pbt_buf, sizeof(CTPM_FW2));
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",
					__func__);
	}
...
u8 fts_ctpm_get_i_file_ver(void)
{
	...
	if((fts_updateinfo_curr.CHIP_ID==0x64)&&(fts_updateinfo_curr.VENDOR_ID==0x11))
	{
		if (ui_sz > 2)
		{
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	pr_info("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n", uc_tp_fm_ver, uc_host_fm_ver);
...
int fts_6x36u_ctpm_fw_upgrade(struct i2c_client *client, u8 *pbt_buf,
			  u32 dw_lenth)
{
	u8 reg_val[2] = {0};
	u32 i = 0;
				       ...
				       char *firmware_name)
{
	...
	{
		i_ret = fts_6x36u_ctpm_fw_upgrade(client, pbt_buf, fwsize);
	}
	else if (fts_updateinfo_curr.CHIP_ID==0x54)
	{

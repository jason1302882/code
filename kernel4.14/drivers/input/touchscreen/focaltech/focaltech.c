...
//static struct wake_lock prx_wake_lock_ps; //chenjiaxi add
 ...
 struct Upgrade_Info fts_updateinfo[] =
{
       ...
       {0x64,0x11,"FT6x36U",TPD_MAXPOINTS_10,AUTO_CLB_NONEED,10, 10, 0x79, 0x1c, 10, 2000},
       {0x64,0x7a,"FT6x36U",TPD_MAXPOINTS_10,AUTO_CLB_NONEED,10, 10, 0x79, 0x1c, 10, 2000},
	...
	{0x0a,"FT5x16",TPD_MAX_POINTS_5,AUTO_CLB_NEED,50, 30, 0x79, 0x07, 10, 1500},
        {0x06,"FT6x06",TPD_MAXPOINTS_10,AUTO_CLB_NONEED,100, 30, 0x79, 0x08, 10, 2000},
        {0x36,"FT6x36",TPD_MAXPOINTS_10,AUTO_CLB_NONEED,10, 10, 0x79, 0x18, 10, 2000},
        {0x64,"FT6x36U",TPD_MAXPOINTS_10,AUTO_CLB_NONEED,10, 10, 0x79, 0x1c, 10, 2000},
...
static int fts_update_data(void)
{
...
#endif
	ret = fts_i2c_Read(this_client, buf, 1, buf, 62);
	...
	for(i = 0; i < TS_MAX_FINGER; i++) {
		...
		#endif
                       input_report_abs(data->input_dev, ABS_MT_POSITION_X,1920-y);//y
                       input_report_abs(data->input_dev, ABS_MT_POSITION_Y, x);//1200-x
                       //1920-y  //1920-x //1200-x //x      //x       //1920-x  //1200-x
                       //1200-x  //1200-y //1920-y //1920-y //1200-y  //y       //y
...
int ft5x0x_prox_ctl(int value)
{
    ...
    if(value == 0){
		//wake_unlock(&prx_wake_lock_ps); //chenjiaxi add
		...
		//wake_lock(&prx_wake_lock_ps); //chenjiaxi add
...
static int ts_adf_event_handler(struct notifier_block *nb, unsigned long action, void *data)
{
	...
	PRINT_DBG("receive adf event with adf_event_data=%d", adf_event_data);

	switch (adf_event_data) {
	case DRM_MODE_DPMS_ON:
		fts_resume(); /* 替换成具体使用的 resume 函数 */
		break;
	case DRM_MODE_DPMS_OFF:
		fts_suspend(); /* 替换成具体使用的 suspend 函数 */
		break;
	default:
		PRINT_WARN("receive adf event with error data, adf_event_data=%d",
			adf_event_data);
		break;
	}
...
if (err < 0)
	PRINT_WARN("register adf notifier fail, cannot sleep when screen off");
else
	PRINT_DBG("register adf notifier succeed");
...
fts_create_sysfs(client);
    ...
    //wake_lock_init(&prx_wake_lock_ps, WAKE_LOCK_SUSPEND, "prx_wake_lock"); //chenjiaxi add

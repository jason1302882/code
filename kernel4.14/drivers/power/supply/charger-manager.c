...
/*	if (!strncmp(env_str, event, UEVENT_BUF_SIZE))
	{
		printk("--->uevent_notify, status not changed!\n");
		return;
	}*/
...
static void usb_charge_works(struct work_struct *work)
{
	int	i;
	int ret;
	struct charger_manager *cm;
	...
	list_for_each_entry(cm, &cm_list, entry)
	{
		for (i = 0; cm->desc->psy_charger_stat[i]; i++)
		{
			...
			if (psy)
			{
				dev_err(cm->dev, "--->usb_charge_works, find power supply \"%s\"\n",	cm->desc->psy_charger_stat[i]);
				break;
			}
		}
		
		if (match_string(cm->desc->psy_charger_stat, -1, psy->desc->name) >= 0 || match_string(&cm->desc->psy_fuel_gauge, -1, psy->desc->name) >= 0)
		{
	...
	{
		gpiostate = gpio_get_value(usb_det_pin);
		if (0 == gpiostate)
		{
			printk("-------->usb_charge_works,plugin!\n");

			if (cm_suspended)
			{
				printk("--->usb_charge_works! 1\n");
				device_set_wakeup_capable(cm->dev, true);
			}
			...
			if (ret)
			{
				printk("--->usb_charge_works! set online fail!\n");
			}
			...
			if (cm->desc->force_set_full)
			{
				printk("--->usb_charge_works! 3\n");
				cm->desc->force_set_full = false;
			}
		
			if (cm->charging_status)
			{
				printk("--->usb_charge_works! 4\n");
				cm->charging_status = 0;
			}
		
			if (cm->emergency_stop)
			{
				printk("--->usb_charge_works! 5\n");
				cm->emergency_stop = 0;
			}
		
			if (is_polling_required(cm) && cm->desc->polling_interval_ms)
			{
				printk("--->misc_event_handler! 6\n");
				schedule_work(&setup_polling);
			}
			uevent_notify(cm, "CHARGING");
			power_supply_changed(cm->charger_psy);
			
			ret = irq_set_irq_type(usb_irq,IRQ_TYPE_LEVEL_HIGH);
			if (ret < 0)
			{
					printk("-------->irq_set_irq_type failed:%d!\n", ret);
					return;
			}		
			enable_irq(usb_irq);
		}
		...
		{
			printk("-------->usb_charge_works,plugout!\n");
			...
			ret = irq_set_irq_type(usb_irq,IRQ_TYPE_LEVEL_LOW);
			if (ret < 0)
			{
					printk("-------->irq_set_irq_type failed:%d!\n", ret);
					return;
...
{
	printk("-------->usb_irq_handler!\n");
	disable_irq_nosync(usb_irq);
	queue_work(usb_workqueue, &usb_work);
	return IRQ_HANDLED;
}
...
static struct charger_desc *of_cm_parse_desc(struct device *dev)
{
	...
	usb_det_pin = of_get_named_gpio(np, "sprd,usb-dc-gpios", 0);
	if (0 > usb_det_pin)
	{
		printk("--->can't read gpio!\n");
	}
	else
	{
		printk("--->read gpio:%d!\n",usb_det_pin);
		ret = gpio_request(usb_det_pin, "usbcharge_detect");
		if (!ret)
		{
			gpio_direction_input(usb_det_pin);
			usb_irq = gpio_to_irq(usb_det_pin);
			if (ret < 0)
			{
			...
			printk("--->set irq type!\n");
			ret = request_irq(usb_irq, usb_irq_handler, IRQF_TRIGGER_LOW | IRQF_NO_SUSPEND, "usb_charge_detect", NULL);
			if (ret < 0) 
			{
				printk("--->usb irq request_irq failed!%d\n", ret);
			}
			
			INIT_WORK(&usb_work, usb_charge_works);
			usb_workqueue = create_singlethread_workqueue("usb_charge_detect");
		}
	}	

...
static int sprd_pwm_backlight_update(struct backlight_device *bd)
{
  ...
  if (bl->levels)
  	bd->props.brightness = bl->levels[brightness];	
			...
			struct sprd_backlight *bl)
{
	...
	prop = of_find_property(node, "sprd,brightness-levels", &length);
	if (!prop)
		return -EINVAL;
	max_num = length / sizeof(u32);
	if(max_num > 0){
		size_t size = sizeof(*bl->levels) * max_num;
		bl->levels = devm_kzalloc(dev, size, GFP_KERNEL);
	 	if (!bl->levels)
	 		return -ENOMEM;
		ret = of_property_read_u32_array(node, "sprd,brightness-levels", bl->levels, max_num);
	 	if (ret < 0)
	 		return ret;
	}

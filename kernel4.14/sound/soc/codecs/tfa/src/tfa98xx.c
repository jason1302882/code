...
void tfa98xx_deferred_calibration_status(Tfa98xx_handle_t handle, int calibrateDone)
{
	...
	if (calib->wr_value) {
		...
		case 1:
			...
			break;
		case 0:
			printk("Calibration not complete, still waiting...\n");
			break;
		case -1:
			...
			break;
		default:
			printk("Unknown calibration status: %d\n",
							calibrateDone);
		}
	}
}
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if(ret) {
		printk("Calibration start failed (%d), deferring...\n", ret);
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if (!strncmp(buf, start_cmd, sizeof(start_cmd) - 1)) {
		printk("Manual triggering of dsp start...\n");
		mutex_lock(&tfa98xx->dsp_lock);
...
static struct snd_soc_dapm_widget tfa98xx_dapm_widgets_common[] = {
	...
	/* Virt dai for voice */
	SND_SOC_DAPM_AIF_IN("AIF IN Voice", "AIF Playback Voice", 0,
			    SND_SOC_NOPM, 0, 0),
	SND_SOC_DAPM_AIF_OUT("AIF OUT Voice", "AIF Capture Voice", 0,
			     SND_SOC_NOPM, 0, 0),
...
static void tfa98xx_tapdet_check_update(struct tfa98xx *tfa98xx)
{
	...
	if (!tfa98xx->tapdet_enabled && enable) {
		...
		err = tfa98xx_dsp_write_mem_word(tfa98xx->handle, XMEM_TAP_ACK, 0,
							Tfa98xx_DMEM_XMEM);
		if (err)
			printk("Unable to write to XMEM\n");
		...
		if (count > 1)
			printk("Had to run %d times to ack SPKS at init\n", count);
	}
...
static void tfa98xx_tapdet(struct tfa98xx *tfa98xx)
{
	...
	switch (mem) {
	case 0xffffffff:
		printk("More than 4 taps detected! (flagTapPattern = -1)\n");
		btn = BTN_0;
		break;
	case 0xfffffffe:
		printk("Single tap detected! (flagTapPattern = -2)\n");
		btn = BTN_0;
		break;
	case 0:
		printk("Unrecognized pattern! (flagTapPattern = 0)\n");
		btn = BTN_0;
		break;
	default:
		printk("Detected pattern: %d\n", mem);
		btn = BTN_0 + mem;
		break;
	}
	...
	if (count > 1)
		printk("Had to run %d times to ack SPKS\n", count);
...
static void tfa98xx_interrupt(struct work_struct *work)
{
	...
	if (out2) {
		...
		if (TFA_GET_BF_VALUE(tfa98xx->handle, ISTMWSRC, out2) > 0) {
			...
			if (manwait1 > 0) {
				printk("entering wait for source state\n");
				tfa98xx->count_wait_for_source_state++;
				
				/* set AMPC and AMPE to make sure the amp is enabled */
				printk("setting AMPC and AMPE to 1 (default) \n");	
				...
				printk("leaving wait for source state\n");
						...
						struct snd_soc_dai *dai)
{
	...
	if(!strcmp(dai->driver->name, "tfa98xx-aif-voice-4-34"))
		tfa98xx_profile = 2;
	else if(!strcmp(dai->driver->name, "tfa98xx-aif-4-34"))
		tfa98xx_profile = 0;
	...
	printk("%s ======== basename = %s, dai_name = %s,  tfa98xx_profile = %d \n", __FUNCTION__, basename, dai->driver->name, tfa98xx_profile);
	...
	printk("tfa98xx_startup   snd_pcm_hw_constraint_list = %d\n", ret);
	return ret;
}
...
static struct snd_soc_dai_driver tfa98xx_dai[] = {
	...
	#if 1
	{
		.name = "tfa98xx-aif-voice",
		.base = TFA98XX_TDM_CONFIG0 - 1,
		.id = 1,
		.playback = {
			.stream_name = "AIF Playback Voice",
			.channels_min = 1,
			.channels_max = 4,
			.rates = TFA98XX_RATES,
			.formats = TFA98XX_FORMATS,
...
static void tfa98xx_irq_9888(struct tfa98xx *tfa98xx)
{
	...
	printk("interrupt1: 0x%.4x (enabled: 0x%.4x)\n", out1, en1);
	printk("interrupt2: 0x%.4x (enabled: 0x%.4x)\n", out2, en2);
	printk("interrupt3: 0x%.4x (enabled: 0x%.4x)\n", out3, en3);
...
static int tfa98xx_parse_dt(struct device *dev, struct tfa98xx *tfa98xx,
	...
	if (tfa98xx->reset_gpio < 0)
		printk("No reset GPIO provided, will not HW reset device\n");

	tfa98xx->irq_gpio =  of_get_named_gpio(np, "irq-gpio", 0);
	if (tfa98xx->irq_gpio < 0)
		printk("No IRQ GPIO provided.\n");

	printk("cjc ==== tfa98xx->reset_gpio = %d, tfa98xx->irq_gpio = %d\n", tfa98xx->reset_gpio,tfa98xx->irq_gpio);
	return 0;
}
